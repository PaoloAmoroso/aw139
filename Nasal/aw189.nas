...
#Thanks to Melchior for a wonderful heli-two-engine code! Unfortunately it doesn't come close to the aw189- it would be much easier and better to have engine-support in the YASim helicopter-code. So any volunteers outthere?
...
# liveries =========================================================
#aircraft.livery.init("Aircraft/aw189/Models/liveries");
...
aircraft.light.new("sim/model/aw189/lighting/strobe-top", [0.05, 1.00], strobe_switch);
aircraft.light.new("sim/model/aw189/lighting/strobe-bottom", [0.05, 1.03], strobe_switch);

# beacons ===========================================================
var beacon_switch = props.globals.initNode("controls/lighting/beacon", 1, "BOOL");
aircraft.light.new("sim/model/aw189/lighting/beacon-top", [0.62, 0.62], beacon_switch);
aircraft.light.new("sim/model/aw189/lighting/beacon-bottom", [0.63, 0.63], beacon_switch);
...
var nav_lights = props.globals.getNode("sim/model/aw189/lighting/nav-lights", 1);
...
var Doors = {
	new: func {
		...
		m.active = 0;
		m.list = [];
		foreach (var d; props.globals.getNode("sim/model/aw189/doors").getChildren("door"))
...
var turbine = props.globals.getNode("sim/model/aw189/turbine-rpm-pct", 1);
var torque_pct = props.globals.getNode("sim/model/aw189/torque-pct", 1);
...
var pendulum = props.globals.getNode("/sim/model/aw189/absorber-angle-deg", 1);
...
var Skid = {
	new: func(n) {
		var m = { parents: [Skid] };
		var soundN = props.globals.getNode("sim/model/aw189/sound", 1).getChild("slide", n, 1);
...
var volume = props.globals.getNode("sim/model/aw189/sound/volume", 1);
...
var crash = func {
	if (arg[0]) {
		# crash
		setprop("sim/model/aw189/tail-angle-deg", 35);
		setprop("sim/model/aw189/shadow", 0);
		setprop("sim/model/aw189/doors/door[0]/position-norm", 0.2);
		setprop("sim/model/aw189/doors/door[1]/position-norm", 0.9);
		setprop("sim/model/aw189/doors/door[2]/position-norm", 0.2);
		setprop("sim/model/aw189/doors/door[3]/position-norm", 0.6);
		setprop("sim/model/aw189/doors/door[4]/position-norm", 0.1);
		setprop("sim/model/aw189/doors/door[5]/position-norm", 0.05);
		...
		# uncrash (for replay)
		setprop("sim/model/aw189/tail-angle-deg", 0);
		setprop("sim/model/aw189/shadow", 1);
		doors.reset();
...
var rotor_step = props.globals.getNode("sim/model/aw189/rotor-step-deg");
...
var doors = Doors.new();
#var config_dialog = gui.Dialog.new("/sim/gui/dialogs/aw189/config/dialog", "Aircraft/aw189/Dialogs/config.xml");
